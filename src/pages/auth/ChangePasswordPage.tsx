...
export function ChangePasswordPage() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#05070a] px-4 py-10">
      ...
      <div className="pointer-events-none absolute left-[9%] top-1/2 hidden w-[15rem] -translate-y-1/2 rounded-[1.75rem] border border-white/8 bg-white/[0.03] p-5 text-left shadow-[0_20px_60px_rgba(0,0,0,0.22)] backdrop-blur-md xl:block">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8c97a5]">Access Policy</p>
        <h3 className="mt-3 text-lg font-bold text-[#f5f7fa]">A password reset keeps admin and client access protected.</h3>
        <div className="mt-4 space-y-3 text-sm text-[#a6b0bd]">
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">Use a strong password that is unique to this portal.</div>
          <div className="rounded-2xl border border-white/8 bg-white/[0.03] p-3">Changing it immediately prevents reuse of temporary credentials.</div>
        </div>
      </div>
      <div className="relative w-full max-w-md">
        ...
        <div className="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(17,23,30,0.94)_0%,rgba(7,10,14,0.98)_100%)] p-8 shadow-[0_30px_80px_rgba(0,0,0,0.45),inset_0_1px_0_rgba(255,255,255,0.06)] backdrop-blur-xl">
          {!user?.mustChangePassword ? (
            <button
              ...
              className="group mb-8 inline-flex items-center gap-1.5 text-sm text-[#9ca7b5] transition-colors hover:text-[#f2f5f7]"
            >
              <ArrowLeft className="h-4 w-4 transition-transform group-hover:-translate-x-0.5" />
              Back to Profile
            </button>
          ) : (
            <div className="mb-6 flex items-start gap-3 rounded-xl border border-[#8b4b49]/70 bg-[linear-gradient(180deg,rgba(71,24,24,0.88)_0%,rgba(42,15,15,0.96)_100%)] p-4">
              <AlertTriangle className="mt-0.5 h-5 w-5 shrink-0 text-[#f0aaa3]" />
              <div>
                <p className="text-sm font-semibold text-[#f3c2bc]">Password change required</p>
          ...
          <div className="mb-6 flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#d6b36a]/30 bg-[linear-gradient(180deg,rgba(214,179,106,0.18)_0%,rgba(214,179,106,0.06)_100%)]">
              <Lock className="h-5 w-5 text-[#e2c98f]" />
            </div>
            <div>
              <h1 className="text-lg font-bold text-[#f5f7fa]">Change Password</h1>
              <p className="text-sm text-[#98a3b2]">Update your account password.</p>
            </div>
          </div>

          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            <div className="space-y-1.5">
              <Label htmlFor="currentPassword" className="text-[13px] font-medium text-[#d8dee6]">
                Current Password
              </Label>
        ...
        {!user?.mustChangePassword && (
          <p className="mt-6 text-center text-sm text-[#98a3b2]">
            <Link
              to="/account/security"
              className="text-[#d6b36a] underline underline-offset-4 transition-colors hover:text-[#f0d28f]"
            >
      ...
      <div className="pointer-events-none absolute right-[9%] top-1/2 hidden w-[14rem] -translate-y-1/2 rounded-[1.75rem] border border-white/8 bg-white/[0.03] p-5 text-left shadow-[0_20px_60px_rgba(0,0,0,0.22)] backdrop-blur-md xl:block">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#8c97a5]">Security Status</p>
        <div className="mt-4 rounded-2xl border border-[#d6b36a]/18 bg-[#d6b36a]/8 p-3">
          <p className="text-sm font-semibold text-[#f2d9a1]">Credential Update</p>
          <p className="mt-1 text-xs leading-5 text-[#c2cbd6]">After the password is changed, the account can continue into dashboard and management flows.</p>
        </div>
      </div>
    </div>
  );
