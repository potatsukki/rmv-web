...
import { LEGACY_SERVICE_REDIRECTS, SERVICE_CATALOG, getServiceById, type ServiceProject } from '@/lib/service-catalog';
...
export function ServiceDetailsPage() {
  ...
  const projects: ServiceProject[] = sharedVariants?.map((variant) => ({
    ...
    detailGroups: variant.confirmationGroups.map((group) => ({
      title: group.title,
      items: group.items.map((item) => `${item.label}: ${item.value}${item.note ? ` — ${item.note}` : ''}`),
    })),
  })) ?? service.projects;
  service.projects = projects;
  const quoteTarget = user ? `/appointments/book?serviceType=${service.serviceType}` : '/login';
  const currentProject = selectedProject === null ? null : projects[selectedProject];
  const designRequestTarget = currentProject && user
    ? `/appointments/book?serviceType=${service.serviceType}&design=${encodeURIComponent(currentProject.title)}`
    : '/login';
  ...
  return (
    <div className="min-h-screen overflow-x-clip bg-[#090B0D] text-white selection:bg-[#F5B400]/30">
      ...
      <main>
        ...
        <section className="relative overflow-hidden bg-[#F5B400] py-14 text-[#090B0D]"><img src={service.coverImage} alt="" className="absolute inset-0 h-full w-full object-cover opacity-10 mix-blend-multiply" /><div className="relative mx-auto grid w-[min(calc(100%_-_32px),1240px)] gap-8 sm:w-[min(calc(100%_-_48px),1240px)] lg:grid-cols-[1fr_0.8fr_auto] lg:items-center"><div><p className="text-xs font-extrabold uppercase tracking-[0.15em]">Start your project</p><h2 className="mt-3 font-['Sora',sans-serif] text-[clamp(2rem,4vw,4rem)] font-extrabold uppercase leading-[0.95] tracking-[-0.04em]">Talk to RMV about your {service.label.toLowerCase()} project.</h2></div><div className="grid gap-3 text-sm font-semibold"><a href="tel:029506187" className="flex items-center gap-3"><Phone className="h-5 w-5" />{OFFICE.tel} / {OFFICE.mobile}</a><a href={`mailto:${OFFICE.email}`} className="flex items-center gap-3"><Mail className="h-5 w-5" />{OFFICE.email}</a><a href={OFFICE.directionsUrl} target="_blank" rel="noreferrer" className="flex items-center gap-3"><MapPin className="h-5 w-5" />Workshop directions</a><p className="flex items-center gap-3"><Clock3 className="h-5 w-5" />{OFFICE.hours}</p></div><Link to={quoteTarget} className="inline-flex min-h-[52px] items-center justify-center gap-3 rounded-md bg-[#090B0D] px-7 text-xs font-extrabold uppercase tracking-[0.12em] text-white transition hover:-translate-y-0.5">Request a Quote<ArrowRight className="h-4 w-4" /></Link></div></section>
